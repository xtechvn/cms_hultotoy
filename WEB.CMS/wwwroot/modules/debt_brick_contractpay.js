...
$(document).ready(function () {
    ...
    $('input').keyup(function (e) {
        if (e.which === 13) {
            _debt_brick_contractpay_service.OnPaging(1);
        }
    });
    $('input').attr('autocomplete', 'off');
    //multi select
    ...
    itemsStatusType.forEach(item => {
        item.addEventListener("click", () => {
            ...
            _debt_brick_contractpay_service.SearchParam.statusMulti = listStatusType
    ...
    //end multi select
    var SearchParam = _debt_brick_contractpay_service.GetParam()
    _debt_brick_contractpay_service.Init(SearchParam);
...
var _debt_brick_contractpay_service = {
    ...
    Export: function () {
        ...
        $.ajax({
            url: "/DebtBrick/ExportExcelContractPay",
            type: "Post",
    ...
    Search: function (input, is_count_status = true) {
        ...
        _global_function.AddLoading()
        $.ajax({
            url: "/DebtBrick/SearchContractPay",
            type: "Post",
            data: input,
            success: function (result) {
                _global_function.RemoveLoading()
    ...
    AddWithContractPay: function (payId, clientId, payment, amount, billNo) {
        let title = 'Gạch nợ phiếu thu';
        let url = '/DebtBrick/AddWithContractPay';
        var param = {
            'clientId': clientId,
            'contractPayId': payId,
            'billNo': billNo,
    ...
    UndoDebtContractPay: function (payId, clientId, payment, amount, billNo) {
        let title = 'Bở gạch nợ phiếu thu';
        let url = '/DebtBrick/UndoDebtBrickContractPay';
        var param = {
            'clientId': clientId,
            'contractPayId': payId,
            'billNo': billNo,
    ...
    History: function (payId, bIllNo) {
        let title = 'Lịch sử gạch nợ phiếu thu';
        let url = '/DebtBrick/HistoryWithContractPay';
        var param = {
            'contractPayId': payId,
            'bIllNo': bIllNo,
        };
