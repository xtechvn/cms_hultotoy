...
var _productManager = {
    ...
    OnInit: function () {
        _productManager.CheckTimeFromURL();
        $('body').on('click', '.onlocked_product', function () {
            _productManager.OnLockedProduct();
        });
        ...
        _productManager.SingleDatePicker($('#ip_search_fromdate'), 'down')
        _productManager.SingleDatePicker($('#ip_search_todate'), 'down')
        ...
        $("#ip_search_product_name").keyup(function (e) {
            if (e.which === 13) {
                _productManager.OnChangeProductName(e.target.value);
            } else {
        ...
        $("#ip_search_product_code").keyup(function (e) {
            if (e.which === 13) {
                _productManager.OnChangeProductCode(e.target.value);
            } else {
        ...
        $('.cbk_search_label, .rad_trade_status, .ckb-news-cate').click(function () {
            var objParam = _productManager.CreateFilterObject();
            _productManager.FilterObject = objParam;
            _productManager.Search(objParam);
        });
        ...
        $('#ip_reset_category').click(function () {
            var seft = $(this);
            _productManager.OnResetInput(seft);
            $("#panel-category-filter li").removeClass("mfp-hide");
            $("#panel-category-filter li a.btn-toggle-cate").removeClass('minus').addClass('plus');
            $("#panel-category-filter ul.lever2").hide();
        });
        ...
        $('#panel-category-filter .ckb-news-cate').click(function () {
            ...
            if (seft.is(":checked")) {
                ...
                var parent_text = _productManager.FindParentOfCate(seft.parent(), text);
        ...
        $('#grid-data').on('click', '.sortable', function () {
            ...
            _productManager.OnSortTable(sortField, sortType);
        });
