...
namespace Repositories.ShippingFeeRepositories.LabelFee
{
    ...
    /// </summary>
    class FeeAmazon : BaseAmazonShippingFee
    {
        //Inject đầu vào
        public FeeAmazon(ProductBuyerViewModel ProductBuyer, List<PriceLevelViewModel> PriceLevelLabel, List<IndustrySpecialViewModel> IndustrySpecial)
        ...
        public Dictionary<string, double> getAmazoneShippingFee()
        {
            try
            {
                var shipping_fee = new Dictionary<string, double>
                {
                    ...
                    { FeeBuyType.PRICE_LAST.ToString(),FirstPoundFee() + NextPoundFee() + LuxuryFee() + ProductBuyer.Price}
                };
                return shipping_fee;
            }
            catch (Exception ex)
