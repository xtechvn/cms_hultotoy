    ...
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task<GenericViewModel<ProductViewModel>> GetProductPagingList(ProductFilterModel filter)
        {
            try
            {
                ...
                {
                    sort_clauses.Add(new FieldSort { Field = "update_last", Order = (Nest.SortOrder)filter.SortType });
                }
                else if (filter.SortField == 1)
                {
                    sort_clauses.Add(new FieldSort { Field = "amount_vnd", Order = (Nest.SortOrder)filter.SortType });
                }
                else if (filter.SortField == 2)
                {
                    sort_clauses.Add(new FieldSort { Field = "product_bought_quantity", Order = (Nest.SortOrder)filter.SortType });
                }
                else if (filter.SortField == 3)
                {
                    sort_clauses.Add(new FieldSort { Field = "shiping_fee", Order = (Nest.SortOrder)filter.SortType });
                }
        ...
        public async Task<bool> UpdateProductQuantityAndGroupOnElastic(IEnumerable<ESModifyProductModel> ArrProduct)
        {
            try
            {
                IProductESRepository<object> _ESRepository = new ProductESRepository<object>(_DataBaseConfig.Value.Elastic.Host);
